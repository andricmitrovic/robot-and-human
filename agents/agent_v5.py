...
def train():
    ...
    for _ in range(MIN_REPLAY_SIZE):
        action = env.unwrapped.sample_valid_action() - 1
        # print(action)
        next_state, reward, done, _, _ = env.step(action + 1)
        ...
        while True:
            ...
            if random.random() < epsilon:
                ...
                with torch.no_grad():
                    ...
                    valid_mask = torch.tensor(env.unwrapped.get_action_mask(), device=device, dtype=torch.bool)
                    q_values[~valid_mask] = -1e9  # Mask out invalid actions
            ...
            if done:
                total_reward = reward
                # print(state)
                # total_reward = -max(state[26], state[27])
